...
if(isset($_POST["save"])){
    ...
    $db = getDB();
    if(isset($id)){
        $stmt = $db->prepare("UPDATE Products set name=:name, quantity=:quantity, price=:price, description=:description, category=:category, visibility=:visibility where id=:id");
